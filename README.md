...
- **安全なハイブリッド矩形検知** — Vision の Document Segmentation と複数のRectangle候補を、幾何検証・検出器間consensus・時間方向priorで選別し、full-frame近似を除外したうえで、他候補より明確に大きい矩形を優先。大きさが近い、または判断が曖昧な場合は誤った自動cropを行わず元画像を保存し、状態に応じて単一検出とdual probeを切り替えます。
